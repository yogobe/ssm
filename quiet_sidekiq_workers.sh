...
# Should be field 0, not 1, for our shell
get_pid() {
    local __workerinfo=$1
    pid=$(echo ${__workerinfo} |cut -f 0 -d' ')
    echo ${pid}
}

# Returns the number of currently running jobs
# Index 5 for the count seems to be correct
get_running_count() {
    local __workerinfo=$1
...
sidekiq_workers=$(pgrep -af ^sidekiq)
worker_count=0
...
while :
do
    sidekiq_workers=$(pgrep -af ^sidekiq) # Re-get the list of workers
    worker_count=$(pgrep -af ^sidekiq | wc -l)
    workers_idle=0
